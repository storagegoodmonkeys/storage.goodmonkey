#!/usr/bin/env python3
"""
Convert updated annexures to PDF format for client delivery
"""
...
def create_updated_annex1_pdf():
    """Create Updated Annex 1 PDF"""
    
    filename = "/Users/tayyab/Desktop/flick/Updated_Annex1_Detailed_Scope.pdf"
    ...
    # Build PDF
    doc.build(story)
    print(f"✅ Updated Annex 1 PDF created: {filename}")
    return filename


def create_updated_annex2_pdf():
    """Create Updated Annex 2 PDF"""
    
    filename = "/Users/tayyab/Desktop/flick/Updated_Annex2_Application_Flow.pdf"
    doc = SimpleDocTemplate(
        filename,
        pagesize=A4,
    ...
    story.append(Paragraph(
        "This document describes the overall application flow, user journeys, data interactions, and backend architecture for the Flick MVP iOS application, <b>including all additional features requested by Good Monkeys LLC</b>. It provides a comprehensive view of how different modules interact with each other and with the backend infrastructure.",
        normal_style
    ))
    story.append(Spacer(1, 0.2*inch))
    ...
    # Build PDF
    doc.build(story)
    print(f"✅ Updated Annex 2 PDF created: {filename}")
    return filename


if __name__ == "__main__":
    print("Creating Updated Annex 1 PDF...")
    annex1_pdf = create_updated_annex1_pdf()
    
    print("\nCreating Updated Annex 2 PDF...")
    annex2_pdf = create_updated_annex2_pdf()
    
    print(f"\n✅ Both updated annexure PDFs created successfully!")
    print(f"📄 Updated Annex 1: {annex1_pdf}")
    print(f"📄 Updated Annex 2: {annex2_pdf}")
